...
echo "Web3 indexer: при web3_indexer_enabled=true в Terraform — тот же образ, entrypoint /web3-indexer; NATS из remote state (Secret mmo-backend, NATS_URL)."
echo "  Внутренний URL: cd \"$STAGING_DIR\" && tofu output -raw web3_indexer_http"
echo "  Смок: kubectl -n mmo port-forward svc/mmo-web3-indexer 8091:8091 → curl -sf http://127.0.0.1:8091/healthz"
