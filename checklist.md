...
- **Поток трафика:** клиент → `cmd/gateway/main.go` (JWT `/v1/session`: чтение **`mmo_player_last_cell`** при наличии DSN и подстановка `resolve_x/z` в выдаваемый токен — claims **`mmo_has_resolve`**, **`mmo_rx`**, **`mmo_rz`**; иначе дефолт `-resolve-x/z` gateway; в **`/v1/ws`** `ResolvePosition` по этим координатам; старые JWT без `mmo_has_resolve` — дефолт gateway; best-effort **`mmo_session_issue`**, при закрытии WS — **`UpsertPlayerLastCell`** с теми же `(rx,rz)`, что ушли в resolve) → `ResolvePosition` у `cmd/grid-manager` (каталог из Consul) → gRPC `cmd/cell-node` … → симуляция `internal/cellsim` + ECS; метрики: `GET /metrics` на gateway; **`GET /readyz`** — readiness (Ping БД, если DSN есть). Схема БД: миграции **goose** в [`internal/db/migrations`](internal/db/migrations), `RunMigrations` + пул — [`internal/db`](internal/db).
- **Grafana (staging):** дашборд **MMO Backend (staging)** — UID **`mmo-backend-overview`** (панели gateway / cell по **`pod`** для нескольких шардов, grid registry); папка алертов **MMO**, группа правил **`mmo-staging`**: rate RPC `code=error` у grid-registry, rate `apply_input` с `status=err` у cell и gateway (`namespace="mmo"` в запросах).
...
- **БД в кластере (операторы уже стоят):** **CloudNativePG** — namespace `postgresql`, CR `Cluster/postgresql` (2 инстанса, healthy); DSN в Secret `mmo-backend` как **`DATABASE_URL_RW`**. **Gateway** поднимает `pgxpool`, на старте **`RunMigrations`** (goose): **`mmo_session_issue`** (аудит JWT), **`mmo_player_last_cell`** (последняя сота + resolve при отключении WS). **ScyllaDB** — в приложении MMO пока нет клиента *(инфра в namespace `scylla` — см. Phase 0)*.
...
- [x] Опционально **Postgres** (`DATABASE_URL_RW`): аудит выдачи токена в `mmo_session_issue`, чтение **`mmo_player_last_cell`** при **`/v1/session`** и запись координат в JWT (`mmo_has_resolve`, `mmo_rx`/`mmo_rz`), **`mmo_player_last_cell`** при закрытии WS (те же `rx,rz`, что в `ResolvePosition`), **`GET /readyz`** (Ping БД); без DSN — только JWT как раньше (в токене без `mmo_has_resolve` WS использует дефолт resolve gateway)
...
- [ ] Prometheus: полнота метрик по всем сценариям; расширение дашбордов (latency и др.)
