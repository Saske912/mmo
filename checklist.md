...
- **Деплой staging:** `scripts/deploy-staging.sh` (тесты → образ → Harbor → [опц.] Job **`scripts/goose-migrate-job.sh`** → OpenTofu), манифесты в `deploy/terraform/staging/`; переменная **`gateway_skip_db_migrations`** (env **`GATEWAY_SKIP_DB_MIGRATIONS`** на gateway). Образ содержит **`/migrate`**; при Job-only DDL: **`STAGING_RUN_GOOSE_JOB=1`** + пример tfvars [`deploy/terraform/staging/gateway_migrations.auto.tfvars.example`](deploy/terraform/staging/gateway_migrations.auto.tfvars.example); справочный манифест — [`deploy/staging/goose-job.example.yaml`](deploy/staging/goose-job.example.yaml). Смоук `scripts/staging-verify.sh` (registry, **`gateway-api-smoke`**, smoke **`export-npc-persist`**, **`forward-update noop`**, **`split-prepare`**, **`split-drain`**, B2, resolve, ping, gateway **`/healthz`**/**`/readyz`**, ws-smoke). Опционально **`STAGING_VERIFY_READYZ_GOOSE_HEADER=1`** (наличие **`X-MMO-Goose-Version`**). **`migration-dry-run`:** **`STAGING_VERIFY_MIGRATION_DRY_RUN=incluster`** или [`scripts/mmoctl-in-cluster.sh`](scripts/mmoctl-in-cluster.sh). Опционально: **`STAGING_VERIFY_EXPECT_CELL_IDS`**, **`STAGING_VERIFY_RESOLVE_CHECKS`**, **`STAGING_VERIFY_MIGRATE_CELL`**.
- **Staging, smoke (факт):** 2026-03-28 — образ **`85c9f3f`**; `deploy-staging.sh` + `staging-verify.sh` OK (**`gateway-api-smoke`**, ws-smoke, B2). **`target_progress`** / ветки (**`branch_path_*`**, **`mercenary_elite_contract`** в миграции); JSONB-**`inventory`**; **`/readyz`** при БД. Gateway: **`prerequisite_quest_id`**, **`newly_unlocked_quests`**, **`items/remove`**, **`items/transfer`**, **`resolve-preview`**; при расхождении last_cell с resolve — **HTTP 409** до WebSocket upgrade + заголовки **`X-MMO-Last-Cell-Id`** / **`X-MMO-Resolved-Cell-Id`** (метрика **`mmo_gateway_cell_handoff_mismatch_total`**). Job-only DDL: см. **`gateway_migrations.auto.tfvars.example`** + **`STAGING_RUN_GOOSE_JOB=1`** (текущий выкат — gateway применяет goose сам, без Job).
- **Клиент Unity:** каталог **`Unity/`** (рядом с backend в монорепо): `make unity-proto` → `Unity/Assets/MMO/Generated/`; скрипты **`Assets/MMO/Runtime`** (**`MmoGatewayClient`**, **`MmoWorldStreamClient`**); EditMode-тесты **`Assets/MMO/Tests`**; Google.Protobuf DLL в **`Assets/Plugins/GoogleProtobuf`**.
...
- **Поток трафика:** клиент → `cmd/gateway/main.go` (**`/v1/session`**: **`display_name`**, **`stats`**, **`wallet`**, **`inventory`** (JSONB, синк с **`mmo_player_item`**), **`quests`** (**`progress`**, **`target_progress`**, опционально **`prerequisite_quest_id`**), **`items`**; **`GET /v1/me/quests`**, **`POST /v1/me/quest-progress`** (автозавершение, награды, при завершении — **`newly_unlocked_quests`**), **`POST /v1/me/items/add`**, **`/items/remove`**, **`/items/transfer`**, **`GET /v1/me/last-cell`**, **`GET /v1/me/resolve-preview`** (`resolve_x`/`resolve_z` в query или подстановка last-cell/дефолт gateway) по JWT (**`Bearer`** или **`?token=`**); **resolve_x/z**; **`/v1/ws`**. **Tempo / корреляция:** при **`TEMPO_OTLP_GRPC`** — OTLP в [`internal/tracing`](internal/tracing); **`otelgrpc`** на gRPC-клиентах gateway (registry, cell), сервере **cell-node**, сервере grid-manager; исходящий к соте **`ForwardCellUpdate`**, на handoff — **`ForwardNpcHandoff`** (registry); span **`Cell.Update`**; опционально **`MMO_CELL_OTEL_TICK_SPAN`** на тик. При **`MMO_LOG_FORMAT=json`** в Loki — **`trace_id`/`span_id`** для **`slog`** (в т.ч. **`registry_resolve_ok`**, **`cell_join_ok`** на WS). Смоук [`scripts/ws-smoke`](scripts/ws-smoke): **`-session-x/z`**, **`-display-name`**, **`-second-session-x/z`**, **`-quest-complete-tutorial`**.
- **Grafana (staging):** дашборд **MMO Backend (staging)** — UID **`mmo-backend-overview`**: панели rate/**p95** (gateway, grid registry, cell tick). Дополнительно импорт JSON **p95 по `method`** для **`mmo_grid_registry_rpc_duration_seconds`**: [`deploy/observability/grafana-dashboard-grid-rpc-p95-by-method.json`](deploy/observability/grafana-dashboard-grid-rpc-p95-by-method.json) (uid **`mmo-grid-rpc-p95`**). Группа алертов **`mmo-staging`**: ошибки RPC/apply_input и **SLO по p95** — **`mmo_gateway_registry_resolve_duration_seconds`** > 0.5s; **`mmo_gateway_cell_join_duration_seconds`** > 0.5s; **`mmo_cell_tick_step_duration_seconds`** > 50ms; **`mmo_grid_registry_rpc_duration_seconds`** (`method="ForwardCellUpdate"`) > 0.5s; **`ForwardNpcHandoff`** (правило **`mmostgp95npch`**, `namespace="mmo"` в PromQL) > 0.5s (окна 5m; серия оживёт после реальных вызовов RPC). Образец PrometheusRule: [`deploy/observability/prometheus-rule-forward-npc-handoff.example.yaml`](deploy/observability/prometheus-rule-forward-npc-handoff.example.yaml); примеры LogQL: [`deploy/observability/loki-logql-traceid.example.txt`](deploy/observability/loki-logql-traceid.example.txt). **Логи:** Terraform **`mmo_structured_logs`** + **`mmo_loki_log_labels`**, outputs **`loki_logql_base`** / **`loki_pod_label_selectors`**, [`internal/logging`](internal/logging).
...
- **Эпик B3 (cold-path):** геометрия и операторский [**runbook**](runbooks/cold-cell-split.md); **MVP handoff NPC:** пошагово **`forward-update`** или **`ForwardNpcHandoff`** из registry / **`mmoctl forward-npc-handoff`** без **`redis-cli`** при пустой целевой соте без игроков. **Gateway:** **`GET /v1/me/resolve-preview`**; при mismatch last_cell ↔ resolve — **409** на **`/v1/ws`** (новая сессия с координатами + реконнект); заголовки **`X-MMO-Last-Cell-Id`** / **`X-MMO-Resolved-Cell-Id`**. Автоматический redirect WS без реконнекта клиента — вне scope.
...
1. **Unity:** игровая сцена (ввод → session → WS), интерполяция снапшотов/дельт, UI для квестов/инвентаря; при 409 — повтор session + reconnect.
2. **Postgres / баланс:** наполнение **`branch_*`** / награды, нагрузочные сценарии; при Job-only DDL держать в CI проверку **`/readyz`** + **`X-MMO-Goose-Version`** после **`goose-migrate-job`**.
3. **Соты:** live-handoff сущностей без полного операторского цикла; §**7** [runbook](runbooks/cold-cell-split.md).
4. **Observability:** импорт **`mmo-grid-rpc-p95`** в общий folder Grafana; **`MMO_CELL_OTEL_TICK_SPAN`** точечно.
...
- [x] **Tempo (опционально):** OTLP при **`TEMPO_OTLP_GRPC`** — [`internal/tracing`](internal/tracing); span’ы: gateway (resolve, join), grid (**`ForwardCellUpdate`**, **`ForwardNpcHandoff`**), cell-node (**`otelgrpc`** + **`Join`/`Leave`/`ApplyInput`/`SubscribeDeltas`**, **`Cell.Update`**, опционально тик при **`MMO_CELL_OTEL_TICK_SPAN`**). **Propagation:** клиенты gateway, dial registry из cell-node, исходящий к соте в registry; JSON-логи — [`internal/logging`](internal/logging); Loki **`trace_id`** ↔ Tempo.
- [x] **Критерий (базово):** Дашборд **p95** и активность; алерты **mmo-staging** (SLO по latency: resolve, join, tick, **ForwardCellUpdate**, **ForwardNpcHandoff** — UID **`mmostgp95npch`**); образец PrometheusRule — [`deploy/observability/prometheus-rule-forward-npc-handoff.example.yaml`](deploy/observability/prometheus-rule-forward-npc-handoff.example.yaml).

#### Следующий шаг (кратко)

**Unity:** сцена + UI поверх **`MmoGatewayClient`** / **`MmoWorldStreamClient`**. **DDL:** `gateway_skip_db_migrations` + **`STAGING_RUN_GOOSE_JOB=1`**. **Смоук:** **`gateway-api-smoke`**, опц. **`STAGING_VERIFY_READYZ_GOOSE_HEADER`**. **Grafana:** импорт **`mmo-grid-rpc-p95`**.
