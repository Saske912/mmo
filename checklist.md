...
- **Клиент Unity:** каталог **`../Unity/`** относительно backend в монорепо: `make unity-proto` → `Unity/Assets/MMO/Generated/`; скрипты **`Assets/MMO/Runtime`** (**`MmoGatewayClient`**, **`MmoWorldStreamClient`**, **`MmoGameBootstrap`**); сцена **`Assets/MMO/Scenes/MmoPlayground.unity`** (первая в Build Settings); меню **`GameObject/MMO/Playground Bootstrap`**; EditMode-тесты **`Assets/MMO/Tests`**; Google.Protobuf DLL в **`Assets/Plugins/GoogleProtobuf`**.
