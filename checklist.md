...
- **Staging, smoke (факт):** обновлять после деплоя (образ = short SHA). `staging-verify.sh`: **`gateway-api-smoke`** (session + **`/v1/me/resolve-preview`**); ws-smoke; **`target_progress`** / ветки квестов в **`mmo_quest_def`**; JSONB-**`inventory`**; **`/readyz`** → **`X-MMO-Goose-Version`** при БД. Gateway: **`prerequisite_quest_id`**, **`newly_unlocked_quests`**, **`items/remove`**, **`items/transfer`**, **`resolve-preview`**; при расхождении last_cell с resolve — **HTTP 409** до WebSocket upgrade + заголовки **`X-MMO-Last-Cell-Id`** / **`X-MMO-Resolved-Cell-Id`** (метрика **`mmo_gateway_cell_handoff_mismatch_total`**).
