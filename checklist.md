...
- **Деплой staging:** `scripts/deploy-staging.sh` (тесты → образ → Harbor → **`goose-migrate-job.sh`** при **`gateway_migrations.auto.tfvars`** с **`gateway_skip_db_migrations = true`** → OpenTofu), манифесты в `deploy/terraform/staging/`; env на gateway **`GATEWAY_SKIP_DB_MIGRATIONS`**. Образ содержит **`/migrate`**; отключить Job в пайплайне: **`STAGING_RUN_GOOSE_JOB=0`**. Пример/дубликат: [`gateway_migrations.auto.tfvars.example`](deploy/terraform/staging/gateway_migrations.auto.tfvars.example); Job-манифест — [`deploy/staging/goose-job.example.yaml`](deploy/staging/goose-job.example.yaml). Смоук `scripts/staging-verify.sh` (registry, **`gateway-api-smoke`**, smoke **`export-npc-persist`**, **`forward-update noop`**, **`split-prepare`**, **`split-drain`**, B2, resolve, ping, gateway **`/healthz`**/**`/readyz`**, ws-smoke). Опционально **`STAGING_VERIFY_READYZ_GOOSE_HEADER=1`**. Отдельно: **`make verify-readyz-goose`** / [`scripts/verify-gateway-readyz-goose.sh`](scripts/verify-gateway-readyz-goose.sh); после деплой-скрипта — **`STAGING_VERIFY_READYZ_GOOSE_AFTER_DEPLOY=1`**. **`migration-dry-run`:** **`STAGING_VERIFY_MIGRATION_DRY_RUN=incluster`** или [`scripts/mmoctl-in-cluster.sh`](scripts/mmoctl-in-cluster.sh). Опционально: **`STAGING_VERIFY_EXPECT_CELL_IDS`**, **`STAGING_VERIFY_RESOLVE_CHECKS`**, **`STAGING_VERIFY_MIGRATE_CELL`**.
- **Staging, smoke (факт):** 2026-03-28 — образ **`2e995e1`**; `deploy-staging.sh` (**`goose-migrate-job`** → **`X-MMO-Goose-Version: 20260428120000`**) + `staging-verify.sh` с **`STAGING_VERIFY_READYZ_GOOSE_HEADER=1`** OK (**`gateway-api-smoke`**, ws-smoke, B2). Контент: **`scholar_deep_archive`** / **`scholar_sigil`**; **`target_progress`** / ветки (**`branch_path_*`**, **`mercenary_elite_contract`**); JSONB-**`inventory`**; **`/readyz`** при БД. Gateway: **`prerequisite_quest_id`**, **`newly_unlocked_quests`**, **`items/remove`**, **`items/transfer`**, **`resolve-preview`**; при расхождении last_cell с resolve — **HTTP 409** до WebSocket upgrade + заголовки **`X-MMO-Last-Cell-Id`** / **`X-MMO-Resolved-Cell-Id`**. **Job-only DDL:** [`gateway_migrations.auto.tfvars`](deploy/terraform/staging/gateway_migrations.auto.tfvars) + **`STAGING_VERIFY_READYZ_GOOSE_AFTER_DEPLOY`** / **`make verify-readyz-goose`**.
- **Клиент Unity:** каталог **`../Unity/`** относительно backend в монорепо: `make unity-proto` → `Unity/Assets/MMO/Generated/`; скрипты **`Assets/MMO/Runtime`** (**`MmoGatewayClient`**, **`MmoWorldStreamClient`**, **`MmoGameBootstrap`**, **`EventSystem`** при старте); сцена **`Assets/MMO/Scenes/MmoPlayground.unity`** (первая в Build Settings); меню **`GameObject/MMO/Playground Bootstrap`**; EditMode-тесты **`Assets/MMO/Tests`** (DTO + handoff preview); Google.Protobuf DLL в **`Assets/Plugins/GoogleProtobuf`**.
