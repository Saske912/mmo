...
- **Staging, smoke (факт):** 2026-03-28 — образ **`0b59d39`**; `staging-verify.sh` + **`STAGING_VERIFY_MIGRATION_DRY_RUN=incluster`** OK; ws-smoke: **`target_progress`** в квестах, JSONB-**`inventory`** синхронизирован с **`mmo_player_item`**; **`/readyz`** при БД отдаёт заголовок **`X-MMO-Goose-Version`**.
...
- **Поток трафика:** клиент → `cmd/gateway/main.go` (**`/v1/session`**: **`display_name`**, **`stats`**, **`wallet`**, **`inventory`** (JSONB, синк с **`mmo_player_item`**), **`quests`** (**`progress`**, **`target_progress`**), **`items`**; **`GET /v1/me/quests`**, **`POST /v1/me/quest-progress`** (автозавершение и награды), **`POST /v1/me/items/add`**, **`GET /v1/me/last-cell`** по JWT (**`Bearer`** или **`?token=`**); **resolve_x/z**; **`/v1/ws`**. **Tempo / корреляция:** при **`TEMPO_OTLP_GRPC`** — OTLP в [`internal/tracing`](internal/tracing); **`otelgrpc`** на gRPC-клиентах gateway (registry, cell), сервере **cell-node**, сервере grid-manager; исходящий к соте **`ForwardCellUpdate`**, на handoff — **`ForwardNpcHandoff`** (registry); span **`Cell.Update`**; опционально **`MMO_CELL_OTEL_TICK_SPAN`** на тик. При **`MMO_LOG_FORMAT=json`** в Loki — **`trace_id`/`span_id`** для **`slog`** (в т.ч. **`registry_resolve_ok`**, **`cell_join_ok`** на WS). Смоук [`scripts/ws-smoke`](scripts/ws-smoke): **`-session-x/z`**, **`-display-name`**, **`-second-session-x/z`**, **`-quest-complete-tutorial`**.
- **Grafana (staging):** дашборд **MMO Backend (staging)** — UID **`mmo-backend-overview`** ([ссылка](https://grafana.pass-k8s.ru/d/mmo-backend-overview?from=now-1h&to=now)): панели rate/**p95** (gateway, grid registry, cell tick). Группа алертов **`mmo-staging`**: ошибки RPC/apply_input и **SLO по p95** — **`mmo_gateway_registry_resolve_duration_seconds`** > 0.5s; **`mmo_gateway_cell_join_duration_seconds`** > 0.5s; **`mmo_cell_tick_step_duration_seconds`** > 50ms; **`mmo_grid_registry_rpc_duration_seconds`** (`method="ForwardCellUpdate"`) > 0.5s (окно 5m). Пример правила под **`ForwardNpcHandoff`**: [`deploy/observability/prometheus-rule-forward-npc-handoff.example.yaml`](deploy/observability/prometheus-rule-forward-npc-handoff.example.yaml); примеры LogQL: [`deploy/observability/loki-logql-traceid.example.txt`](deploy/observability/loki-logql-traceid.example.txt). **Логи:** Terraform **`mmo_structured_logs`** + **`mmo_loki_log_labels`**, outputs **`loki_logql_base`** / **`loki_pod_label_selectors`**, [`internal/logging`](internal/logging).
...
1. **Postgres / геймплей:** больше квестов в каталоге и **сложная логика** (ветвления, цепочки); trade / расход предметов; **Unity** под **`/v1/me/items/add`**, **`last-cell`**, ответ **`quest-progress`** (награды). При Job-only DDL — **`GATEWAY_SKIP_DB_MIGRATIONS`** + **`/migrate`** в CI и контроль **`X-MMO-Goose-Version`**.
2. **Соты:** **live-миграция игроков** между сотами (redirect/handoff в gateway); §**7** [runbook](runbooks/cold-cell-split.md) (drain → **ForwardNpcHandoff** → tfvars) + **GET `/v1/me/last-cell`**. Уже в репо: handoff, кандидаты, drain, импорт/экспорт NPC.
3. **Observability:** завести в Prometheus/Grafana правило по **`ForwardNpcHandoff`** (пример в репо; после первого трафика по RPC появится **`method`** в метриках); при необходимости панель p95 по методу; опционально **`MMO_CELL_OTEL_TICK_SPAN`** только для отладки (объём span’ов).
...
- [x] **Критерий (базово):** Дашборд **p95** и активность; алерты **mmo-staging** (SLO по latency: resolve, join, tick, ForwardCellUpdate); образец правила под **ForwardNpcHandoff** — [`deploy/observability/prometheus-rule-forward-npc-handoff.example.yaml`](deploy/observability/prometheus-rule-forward-npc-handoff.example.yaml) (в Grafana завести по аналогии с **ForwardCellUpdate** после появления вызовов)

#### Следующий шаг (кратко)

Цепочки квестов и trade/расход предметов; **live-migrate** + использование **last-cell**; включить алерт **ForwardNpcHandoff** в **`mmo-staging`** (или PrometheusRule из `deploy/observability`); клиентский контур под новые API.
