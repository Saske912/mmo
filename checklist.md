...
- **Деплой staging:** `scripts/deploy-staging.sh` (тесты → образ → Harbor → OpenTofu), манифесты в `deploy/terraform/staging/`; смоук `scripts/staging-verify.sh` (registry, smoke **`export-npc-persist`** через registry с **`npc_export_json_bytes`>0**, **`forward-update noop`**, **`split-prepare`**, **`split-drain`** на соте из ping + проверка **`Join`** нового игрока, B2, resolve, ping, gateway **`/healthz`** и **`/readyz`**, ws-smoke). **`migration-dry-run`** в смоуке только при **`STAGING_VERIFY_MIGRATION_DRY_RUN=1`** (прямой gRPC к соте по адресу из каталога; с хоста чаще недоступен cluster DNS). Опционально: **`STAGING_VERIFY_EXPECT_CELL_IDS`**, **`STAGING_VERIFY_RESOLVE_CHECKS`**, **`STAGING_VERIFY_MIGRATE_CELL`**.
