...
- **Staging, smoke (факт):** 2026-03-27 — образ **`a77012e`**; `staging-verify.sh` и **`STAGING_VERIFY_MIGRATION_DRY_RUN=incluster`** прошли; ws-smoke видит **`items`** / **`quests`** / **`progress`** в сессии.
...
Расширить геймплей (**цели/награды квестов**, операции **`mmo_player_item`**, согласование с JSONB); **live-migrate** игроков; операторский сплит (**drain → ForwardNpcHandoff** / tfvars) без ручных костылей; **Grafana/alerts** по **`ForwardNpcHandoff`** и span’ы до **Cell.Update**/тика.
