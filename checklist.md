...
- **Деплой staging:** `scripts/deploy-staging.sh` (тесты → образ → Harbor → OpenTofu), манифесты в `deploy/terraform/staging/`; образ содержит **`/migrate`** (опциональный Job — [`deploy/staging/goose-job.example.yaml`](deploy/staging/goose-job.example.yaml); на gateway при Job-only DDL — env **`GATEWAY_SKIP_DB_MIGRATIONS`**). Смоук `scripts/staging-verify.sh` (registry, smoke **`export-npc-persist`**, **`forward-update noop`**, **`split-prepare`**, **`split-drain`**, B2, resolve, ping, gateway **`/healthz`**/**`/readyz`**, ws-smoke). **`migration-dry-run`:** **`STAGING_VERIFY_MIGRATION_DRY_RUN=incluster`** или [`scripts/mmoctl-in-cluster.sh`](scripts/mmoctl-in-cluster.sh). Опционально: **`STAGING_VERIFY_EXPECT_CELL_IDS`**, **`STAGING_VERIFY_RESOLVE_CHECKS`**, **`STAGING_VERIFY_MIGRATE_CELL`**.
- **Staging, smoke (факт):** 2026-03-28 — образ **`cd64dab`**; `staging-verify.sh` + **`STAGING_VERIFY_MIGRATION_DRY_RUN=incluster`** OK; ws-smoke: **`target_progress`** в квестах, JSONB-**`inventory`** синхронизирован с **`mmo_player_item`**; **`/readyz`** при БД отдаёт заголовок **`X-MMO-Goose-Version`**. Gateway: цепочка квестов (**`prerequisite_quest_id`**, **`newly_unlocked_quests`**), **`POST /v1/me/items/remove`**, **`POST /v1/me/items/transfer`**, **`GET /v1/me/resolve-preview`**; при расхождении **`mmo_player_last_cell`** с resolve — счётчик **`mmo_gateway_cell_handoff_mismatch_total`** и заголовки ответа WS (**`X-MMO-Last-Cell-Id`**, **`X-MMO-Resolved-Cell-Id`**).
...
- **Эпик B3 (cold-path):** геометрия и операторский [**runbook**](runbooks/cold-cell-split.md); **MVP handoff NPC:** пошагово **`forward-update`** или **`ForwardNpcHandoff`** из registry / **`mmoctl forward-npc-handoff`** без **`redis-cli`** при пустой целевой соте без игроков. **Gateway (март 2026):** подсказка реконнекта — **`GET /v1/me/resolve-preview`**, метрика/заголовки при расхождении last_cell с resolve; подключение всё равно к соте из registry (**автоматический redirect без реконнекта клиента** по-прежнему вне scope).
...
1. **Клиент Unity:** контракты под **`POST /v1/me/items/remove`**, **`/items/transfer`**, **`GET /v1/me/resolve-preview`**, поля квестов **`prerequisite_quest_id`** и **`newly_unlocked_quests`**; при необходимости чтение заголовков WS при смене покрытия сетки.
2. **Postgres / геймплей:** расширение каталога (**ветвления**, больше цепочек), осмысленные награды/цели; нагрузочные и инвентарные сценарии. Политика Job-only DDL — по мере роста: **`GATEWAY_SKIP_DB_MIGRATIONS`** + **`/migrate`** + контроль **`X-MMO-Goose-Version`** (напоминание в **`scripts/deploy-staging.sh`**).
3. **Соты:** углубление **live-handoff** (опционально жёсткий отказ в WS до реконнекта при mismatch вместо текущего «join к resolve + сигнал»); §**7** [runbook](runbooks/cold-cell-split.md) без изменений.
4. **Observability:** панель **p95** по **`method`** для **`mmo_grid_registry_rpc_duration_seconds`** (в т.ч. видимость **ForwardNpcHandoff** после смоука); **`MMO_CELL_OTEL_TICK_SPAN`** только точечно.
...
**Unity** под remove/transfer/resolve-preview и новые поля квестов; контент квестов и политика Job-only DDL при необходимости; ужесточение handoff в WS при желании; панель p95 по **`method`** в грид-метриках.
