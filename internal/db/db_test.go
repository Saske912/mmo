...
func TestRunMigrationsAndRecord_integration(t *testing.T) {
	...
	if err := UpsertPlayerLastCell(ctx, pool, "test-last-cell", "cell_root", 10.5, -20.25); err != nil {
		t.Fatal(err)
	}
	...
	if gotCell != "cell_root" || rx != 10.5 || rz != -20.25 {
		t.Fatalf("unexpected row: %q %v %v", gotCell, rx, rz)
	}

	if err := UpsertPlayerLastCell(ctx, pool, "test-last-cell", "cell_q0", -500, -500); err != nil {
		t.Fatal(err)
	}
	err = pool.QueryRow(ctx,
		`SELECT cell_id, resolve_x, resolve_z FROM mmo_player_last_cell WHERE player_id = $1`,
	...
	if gotCell != "cell_q0" || rx != -500 || rz != -500 {
